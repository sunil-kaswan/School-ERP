...
//        return ['id' => Qs::hash($q->id), 'name' => $q->name];
    ...
    {
        $districts = $this->loc->getDistricts($state_id);
        return $data = $districts->map(function($q){
            return ['id' => $q->id, 'name' => $q->name];
        })->all();
    }

    public function get_village($tehsils_id)
    {
        $villages = $this->loc->getVillages($tehsils_id);
        return $data = $villages->map(function($q){
            return ['id' => $q->id, 'name' => $q->name];
        })->all();
    }

    public function get_tehsil($district_id)
    {
        $tehsils = $this->loc->getTehsils($district_id);
        return $data = $tehsils->map(function($q){
            return ['id' => $q->id, 'name' => $q->name];
        })->all();
    }
